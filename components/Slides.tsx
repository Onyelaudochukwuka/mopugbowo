...
const Slides: FC = () => {
  ...
  return (
    <div className="flex w-full items-center justify-center mt-24 z-10 relative">
      <span
        className="w-10 h-10 lg:w-16 lg:h-16 block lg:translate-x-56 cursor-pointer lg:relative absolute -left-6 z-50"
        onClick={() => {
          setImages((prev) => {
            const newArr = [...prev];
            const [a, b] = newArr;
      ...
      <span
        className="w-10 h-10 lg:-translate-x-56 lg:w-16 lg:h-16 block cursor-pointer absolute lg:relative -right-6  z-50"
        onClick={() => {
          setImages((prev) => {
            const newArr = [...prev];
            const [a, ...[c, d]] = newArr;
