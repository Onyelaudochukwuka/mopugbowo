...
const Contact: FC = () => {
  ...
  return (
    <section className="p-3 flex flex-col gap-8">
      <h1 className="text-center relative text-bold text-white font-bold tracking-widest text-2xl before:absolute before:w-24 before:h-1 before:rounded-full  before:bg-white before:mt-8 before:origin-left before:transition-[transform] before:duration-300 hover:before:scale-x-125  cursor-pointer">Contact Us</h1>
      <iframe
        ...
        className="mx-auto w-4/5 h-[500px]" loading="lazy" />
      <div className="flex justify-around gap-12 w-11/12 mx-auto lg:flex-row flex-col">
        {contactDetails.map(({ icon, heading, content }, i) =>
          <span key={Id + i} className="flex flex-col items-center justify-center gap-2 lg:p-3 p-12 bg-curves basis-1/5 w-full flex-1 text-center text-white rounded-lg shadow-[0px_4px_8px_rgba(0,0,0,0.25)]">
            <span className="block w-12 h-12"><Image src={icon} layout="responsive" /> </span>
